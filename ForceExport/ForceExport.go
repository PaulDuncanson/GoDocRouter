// Author: Alan Pierce https://github.com/alangpierce/go-forceexport
// History:
//   pbd - 7/15/2018 - fixed hang condition when function is not found
//                     that occurs with golang 1.10.3
...
func FindFuncWithName(name string) (uintptr, error) {
	for moduleData := &Firstmoduledata; moduleData != nil; moduleData = moduleData.next {
		for _, ftab := range moduleData.ftab {
			...
			//fmt.Println(nextFile) // pbd remove when ready for production!!!!!

			// When main.init is reached we've reached end of symbolic table
			// pbd - Added to avoid hang condition.
			if nextFile == "main.init" {
